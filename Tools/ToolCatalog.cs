        ...
        Tool saveMemoryTool = BuildTool(
            ...
            new
            {
                ...
                properties = new
                {
                    ...
                    tags = new { type = "string", description = "Optional comma-separated tags to help find this later, e.g. \"preferences, project-nova\". Include a scope tag so later searches can tell durable facts from ones scoped to a single task: \"durable\" for anything that should apply broadly/forever (facts about the user's life/work - who they are, what they're building), \"style\" for how the user wants to be treated or collaborated with specifically (communication tone, workflow preferences, corrections about how you should behave - different from a fact about them, the same way a person tracks \"how to work with someone\" separately from \"what I know about them\"), \"strategy\" for a reusable *approach* to a certain shape of problem rather than a fact (e.g. \"when scanning for a category of things over a time window, pull broad and filter yourself rather than guessing keywords\") - save one whenever you notice *how* you solved something, not just what the answer was, since these get automatically checked against every new task before it starts - or \"task:<short-name>\" (e.g. \"task:motorola-application\") for something only true within one specific task, like a number or choice made for that task alone - not something to reuse elsewhere." },
                },
